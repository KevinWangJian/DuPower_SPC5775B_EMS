...
static FUNC( void, SOAD_CODE ) SoAd_EnableSpecificRoutingDetChk( VAR( SoAd_RoutingGroupIdType, AUTOMATIC )  RgId_uo,
 ...
 * only on the socket connection identified by SoConId*/
FUNC( Std_ReturnType, SOAD_CODE ) SoAd_EnableSpecificRouting( VAR( SoAd_RoutingGroupIdType, AUTOMATIC ) id,
                                                              VAR( SoAd_SoConIdType, AUTOMATIC )        SoConId)
{
    ...
    SoAd_EnableSpecificRoutingDetChk(id, SoConId);
...
#endif /* SOAD_DEV_ERROR_DETECT_ENABLE != STD_OFF */
    {
                ...
                lIdxSoCon_uo++)
        {
            ...
            if(lSoAdRoutingGroupSoConStructMap_cpst->routingGrpSoConMap_pst[lIdxSoCon_uo].mappedSoConId_uo == SoConId)
            {
                ...
                /* Enable Routing Group only for linked sockets mapped to Routing Group Id */
...
/**
 **********************************************************************************************************************
 * \Function Name : SoAd_EnableSpecificRoutingDetChk()
 *
 * \function description
 * This function is called from SoAd_EnableSpecificRouting to handle the DET error for function arguments.
 * It covers SWS_SoAd_00715 & SWS_SoAd_00716 requirements
 *
 *  Parameters (in):
 * \param   SoAd_RoutingGroupIdType
                                                                 ...
                                                                 VAR( SoAd_SoConIdType, AUTOMATIC )         SoConId_uo )
{
    ...
    SOAD_DET_REPORT_ERROR( ( FALSE == SoAd_InitFlag_b ), SOAD_ENABLE_SPECIFIC_ROUTING_API_ID, SOAD_E_NOTINIT )
    ...
    SOAD_DET_REPORT_ERROR( ( ( SoAd_CurrConfig_cpst->SoAd_noRoutingGrps_cuo <= RgId_uo ) ||
                             ( SoAd_CurrConfig_cpst->SoAd_noStaticSockets_uo <= SoConId_uo ) ),
                                        SOAD_ENABLE_SPECIFIC_ROUTING_API_ID, SOAD_E_INV_ARG )
