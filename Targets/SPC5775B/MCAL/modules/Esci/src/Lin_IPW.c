...
#define LIN_AR_RELEASE_MINOR_VERSION_IPW_C     0
#define LIN_AR_RELEASE_REVISION_VERSION_IPW_C  3
...
#if (LIN_IPW_VENDOR_ID_C != LIN_VENDOR_ID_ESCI)
    #error "Lin_IPW.c and Lin_ESCI.h have different vendor ids"
#endif
...
#if ((LIN_AR_RELEASE_MAJOR_VERSION_IPW_C    != LIN_AR_RELEASE_MAJOR_VERSION_ESCI) || \
     (LIN_AR_RELEASE_MINOR_VERSION_IPW_C    != LIN_AR_RELEASE_MINOR_VERSION_ESCI) || \
     (LIN_AR_RELEASE_REVISION_VERSION_IPW_C != LIN_AR_RELEASE_REVISION_VERSION_ESCI))
    #error "AutoSar Version Numbers of Lin_Ipw.c and Lin_ESCI.h are different"
#endif
/* Check if current file and Lin_ESCI header file are of the same Software version */
#if ((LIN_SW_MAJOR_VERSION_IPW_C != LIN_SW_MAJOR_VERSION_ESCI) || \
     (LIN_SW_MINOR_VERSION_IPW_C != LIN_SW_MINOR_VERSION_ESCI) || \
     (LIN_SW_PATCH_VERSION_IPW_C != LIN_SW_PATCH_VERSION_ESCI))
    #error "Software Version Numbers of Lin_Ipw.c and Lin_ESCI.h are different"
#endif
/*==================================================================================================
...
{
    return Lin_ESCI_CheckWakeup(u8Channel);
}
...
{
    Lin_ESCI_InitChannel(u8Channel);
}
...
*          the entire frame (including header) is sent with the ESCI_Ipw_SendReponse
*
...
{
    return Lin_ESCI_SendHeader(u8Channel, pcPduInfoPtr);
}
...
{
    Lin_ESCI_SendResponse(u8Channel, pcPduInfoPtr);
}
...
{
    return Lin_ESCI_GoToSleep(u8Channel);
}
...
{
    return Lin_ESCI_GoToSleepInternal(u8Channel);
}
...
{
    return Lin_ESCI_HardwareGetStatus(u8Channel, pu8LinSduPtr);
}

/**
* @brief   Sends a wake up signal to the LIN bus.
* @details This function shall sent a wake up signal to the LIN bus
*          and put the LIN channel in LIN_CH_OPERATIONAL_STATE_U8 state.
*
* @param[in]   u8Channel LIN channel to be addressed.
*
* @return            void.
