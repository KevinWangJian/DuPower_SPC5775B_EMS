...
{
    static VAR(Adc_ValidationResultType, AUTOMATIC) ValidationStatus;
...
)
{
    ...
    VAR(Adc_GroupConvType, AUTOMATIC) GroupType = ADC_CONV_TYPE_NORMAL;
    ...
    NoGroupsInSwInjectedQueue = Adc_aUnitStatus[Unit].SwInjectedQueueIndex;
    /** @violates @ref Adc_c_REF_4 Array indexing */
    GroupType = Adc_pCfgPtr->pGroups[Group].eType;
...
#endif /* ADC_ENABLE_QUEUING == STD_OFF  */
    {
        ...
        if (((ADC_HWTRIGGER_ENABLED == eHwTriggering) && ((0U != NoGroupsInHwInjectedQueue) || (0U != NoGroupsInHwNormalQueue))) || \
            ((ADC_CONV_TYPE_INJECTED == GroupType) && ((0U != NoGroupsInSwInjectedQueue) || (0U != NoGroupsInHwNormalQueue))) || \
            ((ADC_CONV_TYPE_NORMAL == GroupType) && ((0U != NoGroupsInHwInjectedQueue) || (0U != NoGroupsInSwNormalQueue) )) \
           )
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_ENABLE_CHANNEL_ID, Group))
    {
           ...
           )
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateEnableDisbleIdList(ADC_ENABLE_CHANNEL_ID, Group, Channel))
            {
                ...
                {
                    /* Precision channel */
                    Adc_aNCMRxMask[GroupId].Adc_u32NCMR0 = (CMRMask.Adc_u32NCMR0 | (1UL << ChId));
                }
                else if ((ChId >= ADC_ST_EXTENDED_CH_U8) && (ChId <= ADC_ED_EXTENDED_CH_U8))
                {
                    /* Standard channel */
                    Adc_aNCMRxMask[GroupId].Adc_u32NCMR1 = (CMRMask.Adc_u32NCMR1 | (1UL << (ChId - ADC_ST_EXTENDED_CH_U8)));
                }
                else if ((ChId >= ADC_ST_EXTERNAL_CH_U8) && (ChId <= ADC_ED_EXTERNAL_CH_U8))
                {
                    /* External multiplexed channel */
                    Adc_aNCMRxMask[GroupId].Adc_u32NCMR2 = (CMRMask.Adc_u32NCMR2 | (1UL << (ChId - ADC_ST_EXTERNAL_CH_U8)));
                }
                else
                {
                    /* nothing for misra */
...
#if ((ADC_ENABLE_CONFIGURE_THRESHOLD_NONAUTO_API == STD_ON) && defined(ADC_WDG_SUPPORTED)) \
    || defined(__DOXYGEN__)
/**
...
#endif /* #if ((ADC_ENABLE_CONFIGURE_THRESHOLD_NONAUTO_API == STD_ON) && defined(ADC_WDG_SUPPORTED)) \
    || defined(__DOXYGEN__) */
...
(
    ...
    CONST(uint16, AUTOMATIC) u16Mask,
...
)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_SETCHANNEL_ID, Group))
    {
...
#endif /* ADC_DEV_ERROR_DETECT == STD_ON */
        {
            ...
            Adc_aRuntimeGroupChannel[Group].u16Mask = u16Mask;
        #endif /* ADC_PDB_SUPPORTED */
