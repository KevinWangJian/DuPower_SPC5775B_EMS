/**
*   @file    Can_FlexCan.c
*   @implements Can_FlexCan.c_Artifact
*   @version 1.0.1
*
...
* Violates MISRA 2004 Required Rule 10.1, The value of an expression of integer type shall not be implicitly converted to a different
* underlying type.
*
* @section Can_Flexcan_c_REF_12
* Violates MISRA 2004 Required Rule 10.3, The value of a complex expression of integer type shall only be cast to a type that is not
* wider and of the same signedness.
*
...
/* @violates @ref Can_Flexcan_c_REF_7 Violates MISRA 2004 Required Rule 8.10, external ... could be made static */
CONST(uint8, CAN_CONST) u8MessageBufferControllerSize[CAN_CONTROLLER_REFERENCE_COUNTER]={ 
 #ifdef CAN_MB_CONTROLLER_1
    CAN_MB_CONTROLLER_1
 #endif  
  #ifdef CAN_MB_CONTROLLER_2
    ,CAN_MB_CONTROLLER_2
 #endif 
  #ifdef CAN_MB_CONTROLLER_3
    ,CAN_MB_CONTROLLER_3
 #endif 
  #ifdef CAN_MB_CONTROLLER_4
    ,CAN_MB_CONTROLLER_4
 #endif 
  #ifdef CAN_MB_CONTROLLER_5
    ,CAN_MB_CONTROLLER_5
 #endif 
  #ifdef CAN_MB_CONTROLLER_6
    ,CAN_MB_CONTROLLER_6
 #endif 
  #ifdef CAN_MB_CONTROLLER_7
    ,CAN_MB_CONTROLLER_7
 #endif 
  #ifdef CAN_MB_CONTROLLER_8
    ,CAN_MB_CONTROLLER_8
 #endif 
};
/**
...
static FUNC (void, CAN_CODE) Can_FlexCan_SetFDENBitErrata( VAR(uint8, AUTOMATIC) u8HwOffset )
{
#if STD_OFF == CAN_E01368_INCONSISTENT
    VAR(uint32, AUTOMATIC)          u32TimeoutCount = 0U;
#endif

#if STD_ON == CAN_E01368_INCONSISTENT
    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
...
/*================================================================================================*/
/**
* @brief          Service for Rx FIFO initialization in the Can_Flexcan_ChangeBaudrate function
* @details        Service for Rx FIFO initialization in the Can_Flexcan_ChangeBaudrate function
*
*                 This routine is called by:
...
static FUNC (void, CAN_CODE) Can_FlexCan_InitRxFiFo( VAR(uint8, AUTOMATIC) Controller, VAR(uint8, AUTOMATIC) u8HwOffset)
{
    ...
    VAR(uint8, AUTOMATIC)             u8NumFilter[16] = {7U,9U,11U,13U,15U,17U,19U,21U,23U,25U,27U,29U,31U,33U,35U,37U};
    ...
    if ( CAN_CONTROLLERCONFIG_RXFIFO_U32 == (pCanControlerDescriptor->u32Options & CAN_CONTROLLERCONFIG_RXFIFO_U32) )
    {
        ...
        for ( u8TempIndex = u8RxFifoTableIdIndex; u8TempIndex < (u8RxFifoTableIdIndex + (4U * (Can_pCurrentConfig->ControlerDescriptors[Controller].u8RxFiFoUsedMb - 6U))); u8TempIndex++)
        {
            ...
            {
        #endif
              if (u8RxFifoTableId <= u8NumFilter[u8FilterIndex])
              {
...
/*================================================================================================*/
/**
* @brief          Service for Receive Message buffer initialization in the Can_Flexcan_ChangeBaudrate function
* @details        Service for Receive Message buffer initialization in the Can_Flexcan_ChangeBaudrate function
*
*                 This routine is called by:
*                  - Can_Flexcan_ChangeBaudrate()
*
...
/*================================================================================================*/
/**
* @brief          Service for Transmit Message buffer initialization in the Can_Flexcan_ChangeBaudrate function
* @details        Service for Transmit Message buffer initialization in the Can_Flexcan_ChangeBaudrate function
*
*                 This routine is called by:
*                  - Can_Flexcan_ChangeBaudrate()
*
        ...
        if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
        {
            ...
            {   
                ...
                */
                ...
                {
            #endif
                    if((u32MbMessageId == Tester_CAN_ID))
                    {
                    	Tester_TEST.rx_buf[0]=u8CanMbData[0];	//get can data ¡¾8 Bytes¡¿
                    	Tester_TEST.rx_buf[1]=u8CanMbData[1];
                    	Tester_TEST.rx_buf[2]=u8CanMbData[2];
                    	Tester_TEST.rx_buf[3]=u8CanMbData[3];
                    	Tester_TEST.rx_buf[4]=u8CanMbData[4];
                    	Tester_TEST.rx_buf[5]=u8CanMbData[5];
                    	Tester_TEST.rx_buf[6]=u8CanMbData[6];
                    	Tester_TEST.rx_buf[7]=u8CanMbData[7];
                    	Tester_18FFFA01_Processor();
                    }

//                    else if(u32MbMessageId == 0x98220145)
//                    {
//                    	g_bl_HCUOnLine_Flag=1;
//                    	g_u8_HcuTimeoutCounter=0;
//                    }
        ...
        if ( (uint32)0x0U != (u32IflagReg & u32TempFlag) )
        {
            ...
            || ((uint32)u32TempFlag == (u32ImaskReg & u32TempFlag)) )
            {
                ...
                if ( FLEXCAN_FIFOFRAME_INT_INDEX_U8 == (uint32)u8MbIndex )
                {
                    ...
                    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */  
                         (Can_pControlerDescriptors[controller].u8RxFiFoUsedMb <= (uint8)u8MbIndex)
                       )
                    {
                #endif 
                        if(FLEXCAN_MBCS_CODERXOVRR_U32 == (u32MbConfig & FLEXCAN_MBCS_CODE_U32))
...
#if (STD_ON == CAN_ENABLE_USER_MODE_SUPPORT)
/* @violates @ref Can_Flexcan_c_REF_7 Violates MISRA 2004 Required Rule 8.10, external ... could be made static */
/* @violates @ref Can_Flexcan_c_REF_13 Violates MISRA 2004 Required Rule 8.1, Functions shall ... function definition and cal */
FUNC(void, CAN_CODE) Can_FlexCan_SetUserAccessAllowed( VAR(uint32, AUTOMATIC) u8HwOffset )
/* @violates @ref Can_Flexcan_c_REF_13 Violates MISRA 2004 Required Rule 8.1, Functions shall ... function definition and cal */
{
    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
    SET_USER_ACCESS_ALLOWED(u8HwOffset);
}
...
FUNC(void, CAN_CODE) Can_FlexCan_RxFifoFrameAvNotif( CONST(uint8, AUTOMATIC) controller)
{
    ...
    VAR(uint8, AUTOMATIC)    u8MbDataLength = 0U;
    /* Physical data bytes. */
    VAR(uint8, AUTOMATIC)    u8CanMbData[8] = {0x0U, 0x0U, 0x0U, 0x0U, 0x0U, 0x0U, 0x0U, 0x0U};
    /* Index for data byte of MB. */
    VAR(uint8, AUTOMATIC)    u8DatabyteIndex = 0U;
    /* Variable for storing the Hrh. */
