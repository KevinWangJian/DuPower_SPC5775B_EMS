...
#endif
#if ((PWM_DIABLE_TRIGEER_API == STD_ON) || (PWM_ENABLE_TRIGEER_API == STD_ON))
...
/**
* @brief         Pwm_GetIpChannelIndex
* @details       This function is called from Pwm_Ipw.c file in order to get a
*                channel from the IP configuration which triggered the interrup.
*
* @param         Channel - HW channel ID that triggered the interrupt
* @return        uint8
*
* @implements    Pwm_GetIpChannelIndex_Activity
*
*/
/*
* @violates @ref Pwm_C_REF_2 external ... could be made static
...
{
    VAR(uint8, AUTOMATIC) u8MappedIndexed = (uint8)0;
    VAR(uint8, AUTOMATIC) u8LogicalChannel = (uint8)0;
    CONSTP2CONST(Pwm_IpConfigType, AUTOMATIC, PWM_APPL_CONST) pIpConfig = &Pwm_pConfig->IpConfig;

    if(NULL_PTR != Pwm_pConfig)
    {
        u8LogicalChannel = Pwm_pConfig->HwToLogicChannelMap[Channel];
        u8MappedIndexed = (*pIpConfig->pIpChannelsConfig)[u8LogicalChannel].u8IdxChannelConfig;
    }

    return u8MappedIndexed;
}
#endif
...
*
* @return      void
* @implements  Pwm_SelectCommonTimebase_Activity
*/
/*
* @violates @ref Pwm_C_REF_2 external ... could be made static
*/
...
/**
* @brief        This function shall reset the HW counter of the PWM timer
* @details       
*               
*
...
)
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PWM_RESETCOUNTER_ID);
    if((Std_ReturnType)E_OK == CallIsValid)
    {
#endif /* PWM_VALIDATE_GLOBAL_CONFIG_CALL */

#if (PWM_PARAM_CHECK == STD_ON)
        if((Std_ReturnType)E_OK == Pwm_ValidateModuleId((uint8)u8ModuleId, PWM_RESETCOUNTER_ID))
        {        
#endif /* (PWM_PARAM_CHECK == STD_ON) */
           Pwm_Ipw_ResetCounter(u8ModuleId);
#if (PWM_PARAM_CHECK == STD_ON)
        }
#endif /* (PWM_PARAM_CHECK == STD_ON) */
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    ...
    * one side efect however executed, or b)cause control flow to change.
    */
    Pwm_EndValidateGlobalConfigCall(CallIsValid, PWM_RESETCOUNTER_ID);
#endif /* PWM_VALIDATE_GLOBAL_CONFIG_CALL */
}
