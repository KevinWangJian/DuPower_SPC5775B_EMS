...
#define PWM_EMIOS_C_AR_RELEASE_MINOR_VERSION       0
#define PWM_EMIOS_C_AR_RELEASE_REVISION_VERSION    3
#define PWM_EMIOS_C_SW_MAJOR_VERSION               1
#define PWM_EMIOS_C_SW_MINOR_VERSION               0
#define PWM_EMIOS_C_SW_PATCH_VERSION               1
...
*/
static VAR(uint8, PWM_VAR) Pwm_eMios_aChannelNotifType[PWM_EMIOS_HW_CHANNELS_MAX_U8];
...
static VAR(Pwm_PeriodType, PWM_VAR) Pwm_eMios_aDaocChannelDuty[PWM_EMIOS_HW_CHANNELS_MAX_U8];
/**
@brief array used to store the current modulo for each pwm channel for updating the register A in DAOC mode
*/
static VAR(Pwm_PeriodType, PWM_VAR) Pwm_DAOC_aChannelModulo[PWM_EMIOS_HW_CHANNELS_MAX_U8];
#endif
...
#if (PWM_NOTIFICATION_SUPPORTED == STD_ON)
    u8Notification =  Pwm_eMios_aChannelNotifType[u8eMiosChannelIdx];
#endif
...
)
{
    ...
    if (((uint16)0x0U == u16DutyCycle) || ((uint16)0x0U == nTempDuty))
    {
        Pwm_eMios_aDaocChannelDuty[u8eMiosChannelIdx] = (uint16)0U;
        /* force match B */
        /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
        /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
        REG_BIT_SET32(EMIOS_CCR_ADDR32(u8mIdx, u8chIdx), CCR_FORCMB_MASK_U32 );
    ...
    else if (PWM_DUTY_CYCLE_100 == u16DutyCycle)
    {
        ...
        /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
        /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
        REG_READ32( EMIOS_CADR_ADDR32(u8mIdx, (uint32)u8chIdx))) % Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
        ...
        else
        {
            /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
            /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
            REG_WRITE32( EMIOS_CADR_ADDR32(u8mIdx,(uint32)u8chIdx),Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
            /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
            /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
            REG_WRITE32( EMIOS_CBDR_ADDR32(u8mIdx,(uint32)u8chIdx),Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
        }
    ...
    {
        Pwm_eMios_aDaocChannelDuty[u8eMiosChannelIdx] = (uint16)nTempDuty;
    ...
    (PWM_FEATURE_DAOC == STD_ON))
        ...
        { /* Bus A or Bus Diverse */
            ...
            case PWM_MODE_DAOC_U32:
                ...
                Pwm_DAOC_aChannelModulo[u8Counter] = peMiosChannelConfig->nPwmDaocModulo;
...
)
{
    ...
    Pwm_eMios_aChannelNotifType[u8IdxChannelConfig] = (uint8)eNotification;
...
)
{
    ...
    Pwm_eMios_aChannelNotifType[u8IdxChannelConfig] = (uint8)0U;
...
)
{
    ...
    case PWM_MODE_DAOC_U32:
        /* Update new modulo */
        Pwm_DAOC_aChannelModulo[u8IdxChannelConfig] = nPeriod;
        nPeriod = Pwm_eMios_aChannelPeriod[peMiosChannelConfig->u8HwChannel];
...
)
{
    ...
    VAR(uint8, AUTOMATIC)                    u8eMiosChannelIdx;
    ...
    u8mIdx  = (uint8)EMIOS_MODULE_INDEX_U8(u8HwChannel);
    u8chIdx = (uint8)EMIOS_CHANNEL_INDEX_U8(u8HwChannel);
    u8eMiosChannelIdx = Pwm_Ipw_GetHwChannelIndex(u8HwChannel);
...
#if (PWM_NOTIFICATION_SUPPORTED == STD_ON)
    if(u8eMiosChannelIdx != (uint8)0xFFU)
    {
        ...
        if(((uint32)0x0U) != u32TempStatus)
        {
            if(u8eMiosChannelIdx != (uint8)0xFFU)
            {
                ...
                if (PWM_MODE_DAOC_U32 == u32ChMode )
                {
                    ...
                    /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
                    /* @violates @ref Pwm_eMIOS_C_REF_6 Cast of complex integer expression to larger type */
                    u32RegDuty = (uint32)(Pwm_eMios_aChannelPeriod[u8HwChannel] - Pwm_eMios_aDaocChannelDuty[u8eMiosChannelIdx]);
                    /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
                    /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
                    u32RegAValue = (((u32RegDuty) + REG_READ32(EMIOS_CBDR_ADDR32(u8mIdx, (uint32)u8chIdx))) % Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
                    ...
                    /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
                    /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
                    u32RegBValue = ((Pwm_eMios_aChannelPeriod[u8HwChannel]+u32RegDuty)%Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
                ...
                else if(CCR_MODE_DAOC_BOTH_U32 == u32ChMode )
                {
                    ...
                    else
                    {
                        /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
                        /** @violates @ref Pwm_eMIOS_C_REF_16 Cast between a pointer type and an integral type */
                        u32RegBValue = ((Pwm_eMios_aDaocChannelDuty[u8eMiosChannelIdx] + \
                                        REG_READ32(EMIOS_CADR_ADDR32(u8mIdx, (uint32)u8chIdx)))% Pwm_DAOC_aChannelModulo[u8eMiosChannelIdx]);
                        if (u32RegBValue != (uint32)0U)
                        {
                            /** @violates @ref Pwm_eMIOS_C_REF_7 Cast from unsigned long to pointer */
                ...
                {
                    if( Pwm_eMios_aDaocChannelDuty[u8eMiosChannelIdx] != 0x0U )
                    {
...
)
{
    ...
    if(PWM_MODE_DAOC_U32 == u32CtrlMode)
    {
        ...
        {
            nMaxPeriod = (Pwm_PeriodType)Pwm_DAOC_aChannelModulo[u8IdxChannelConfig];
        }
        else
        {
